...
if (needChangeTZ) {
  ...
  ;(function (root, factory) {
    ...
    function OffsetAt(at) {
      let timeString = at.toTimeString()
    ...
    function rebuildGuess() {
      ...
      try {
        ...
        if (intlName && intlName.length > 3) {
          let name = names[normalizeName(intlName)]
          if (name) {
            return name
          }
    ...
    function needsOffset(m) {
      let isUnixTimestamp = m._f === 'X' || m._f === 'x'
  ...
  $.Radiko.Player.View.seekBarView.updateBalloon = function (ftTime, addTime) {
    ...
    let seekmt = moment(ftTime + addTime)
    let hour = seekmt.hour()
    if (hour < 5) {
      hour += 24
    }
  ...
  let onDragSeekKai = function () {
    moveSeek = true
    let seekBar = $('#seekbar')
    let x = seekBar.find('.knob').position().left
    let rate = x / seekBar.width()
    let playTime = player.totm() - player.fttm()
    let time = playTime * rate
    time = time > playTime ? playTime : time

    let seekTime = new XDate(player.fttm() + time, false)
    let now = moment()
    ...
    if (moment(player.fttm() + time + diffTimestamp).isAfter(now)) {
      ...
      let moveTime = now.subtract('minutes', 1).format('x') + diffTimestamp - player.fttm().getTime() //THIS LINE CHANGED
      let timeRate = moveTime / playTime
      let movablePos = seekBar.width() * timeRate
...
store.watch('update', function (key, val, oldVal) {
  // if oldVal === undefined -> a new created one
  if (/[0-9]{14}$/.test(key)) {
    val.listened_time = 0
