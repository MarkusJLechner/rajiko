...
document.addEventListener('DOMContentLoaded', function () {
  ...
  chrome.tabs.executeScript(
      {
        code: "var tmpdata = {href : window.location.href,tmpUrl : document.getElementById('tmpUrl') && document.getElementById('tmpUrl').value, url :document.getElementById('url') && document.getElementById('url').value };tmpdata",
      ...
      function (results) {
        ...
        chrome.storage.local.get({current_recording: false, timeshift_list: []}, function (data) {
          ...
          if (data['current_recording']) {
            ...
            if (shouldhidden) {
              ...
              if (url[0] === '#') {
                ...
                //viewing live
                record_button.hidden = false
                record_button.innerText = chrome.i18n.getMessage(
                    'record_button_to_prepare',
                    radioAreaId[tmpUrl.slice(1)].name,
                )
                record_button.onclick = function (data) {
                  chrome.tabs.query({active: true, currentWindow: true}, function (arrayOfTabs) {
                    let tab = arrayOfTabs[0]
          ...
          if (
              tmpUrl.indexOf('https://radiko.jp/v2/api/ts/playlist.m3u8') !== -1 &&
              /\/ts\//.test(results[0].href) &&
              !data['timeshift_list'].includes(tmpUrl)
          ) {
            //viewing timeshift
