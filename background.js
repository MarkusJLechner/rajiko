...
function retTokenByRadioname(radioname, default_area_id) {
  ...
  if (hadTokenArea.length !== 0) {
    let pickArea = hadTokenArea.includes(default_area_id)
        ? default_area_id
        : hadTokenArea[0]
    return [authTokens[pickArea].token, pickArea]
  }
  let pickArea =
      availableArea[Math.floor(Math.random() * availableArea.length) >> 0]
...
const VERSION_MAP = {
  ...
  '5.1.0': {
    sdk: '22',
    builds: ['LMY47O', 'LMY47M', 'LMY47I', 'LMY47E', 'LMY47D'],
  },
  '5.1.1': {
    sdk: '22',
    builds: [
...
function genRandomInfo() {
  let version =
      ...
      Math.floor(Math.random() * Object.keys(VERSION_MAP).length) >> 0
          ]
  let sdk = VERSION_MAP[version].sdk
  let build =
      VERSION_MAP[version].builds[
      Math.floor(Math.random() * VERSION_MAP[version].builds.length) >> 0
          ]
...
function parseAAC(data) {
  ...
  if (
      processing.getUint8(0) !== 73 ||
      processing.getUint8(1) !== 68 ||
      processing.getUint8(2) !== 51
  ) {
    // ID3
    return [0, 0]
  }
...
function streamListener() {
      ...
      if (data['current_recording']) {
        ...
        if (chrome.webRequest.filterResponseData) {
          ...
          filter.ondata = function (event) {
            ...
            if (audio_uint8 === null) {
              ...
              let tmp = new Uint8Array(
                  audio_uint8.byteLength + event.data.byteLength,
              )
          ...
          filter.onstop = function (event) {
            ...
            let audio_string = ab2str(
                audio_uint8.buffer,
                parseAAC(audio_uint8.buffer)[0],
            ) // drop id3 tag ;timestamp not used for now
            let storage_set = {}

            storage_set[radioname + '_' + start_time + '_' + count] =
                audio_string
            count += 1
          ...
          let ifInit =
              req.initiator &&
              req.initiator.toLowerCase().indexOf('chrome-extension') !== -1 //initiator since chrome 63
          ...
          req.requestHeaders.map(function (x) {
  ...
  let stopmeFunc = function (msg, sender, respCallback) {
    if (msg['stop-recording']) {
      //do stop recording caused by pause button need same tabid?
      respCallback()
      ...
      chrome.storage.local.get('current_recording', function (data) {
        if (data['current_recording']) {
          ...
          let radioname = info['radioname']
          let filename =
              timestamp2Filename(start_time) +
              '_' +
              timestamp2Filename(end_time) +
              '.aac'
          if (count === 0) {
            ...
            if (data) {
              ...
              chrome.downloads.download(
                  ...
                  function (downloadId) {
                    ...
                    chrome.downloads.onChanged.addListener(function handler(
                        delta,
                    ) {
                      if (
                          delta.id === downloadId &&
...
function downloadtimeShift(m3u8link, default_area_id) {
  ...
  retTokenByRadioname_async(
      radioname,
      default_area_id,
      function (token, pickArea) {
...
chrome.storage.local.get({selected_areaid: 'JP13'}, function (data) {
  ...
  let authListener = (function () {
    ...
    let modifyResponse = function (resp) {
      let ifInit =
          resp.initiator &&
          resp.initiator.toLowerCase().indexOf('chrome-extension') !== -1 //initiator since chrome 63
      ...
      for (let i = 0; i < resp.responseHeaders.length; i++) {
        if (
            resp.responseHeaders[i].name.toLowerCase() === 'x-radiko-keyoffset'
        ) {
        ...
        if (
            resp.responseHeaders[i].name.toLowerCase() === 'x-radiko-keylength'
        ) {
  ...
  chrome.runtime.onMessage.addListener(function (msg, sender, respCallback) {
    if (msg['update-area']) {
      ...
      chrome.storage.local.set(
          ...
          function () {
            ...
            chrome.webRequest.onBeforeSendHeaders.addListener(
                //for both (firefox can in onBeforeRequest with blocking,chrome can in onSendHeaders with requestHeaders)
                streamListener(),
                {
                  urls: [
      ...
      chrome.tabs.update(sender.tab.id, {
        url: 'https://radiko.jp/#!/ts/' + param.station + '/' + param.t,
      })
    }
  })
          ...
          for (let i = 0; i < req.requestHeaders.length; i++) {
            if (req.requestHeaders[i].name.toLowerCase() === 'user-agent') {
              ...
              if (ua.indexOf('android') !== -1 || ua.indexOf('mobile') !== -1) {
                req.requestHeaders[i].value = req.requestHeaders[i].value
                    .replace(/android.*?\;/gi, '')
                    .replace(/mobile/gi, '') //ugly
                console.log('mobile adjustments', req.requestHeaders[i].value)
      ...
      function(details) {
        for (var i = 0; i < details.requestHeaders.length; ++i) {
          if (details.requestHeaders[i].name === 'User-Agent') {
            console.log('change user agent')
            details.requestHeaders[i].value = 'desktop'
  ...
  chrome.webRequest.onBeforeRequest.addListener(
      function (req) {
        ...
        let regexpresult =
            /http:\/\/radiko\.jp\/v2\/station\/stream_rpaa\/(.*?)\.xml/g.exec(
                req.url,
            )
        if (regexpresult) {
  ...
  chrome.webRequest.onBeforeSendHeaders.addListener(
      function (req) {
        ...
        if (regexpresult) {
          ...
          req.requestHeaders = req.requestHeaders.filter(function (x) {
            return !['x-radiko-authtoken', 'x-radiko-areaid'].includes(
                x.name.toLowerCase(),
      ...
      {
        urls: [
          '*://f-radiko.smartstream.ne.jp/*/_definst_/simul-stream.stream/playlist.m3u8?*',
        ], // "*://f-radiko.smartstream.ne.jp/*/_definst_/simul-stream.stream/chunklist_*.m3u8"
      },
      ['blocking', 'requestHeaders'],
  )
  ...
  chrome.webRequest.onHeadersReceived.addListener(
      function (req) {
        for (let i = 0; i < req.responseHeaders.length; i++) {
          if (
              req.responseHeaders[i].name.toLowerCase() ===
